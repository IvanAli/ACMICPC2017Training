...
void update(int p, int v, int id = 1, int l = 0, int r = len) {
	if (l + 1 == r) {
		segtree[id] = v;
		return;
	}
...
int query(int x, int y, int id = 1, int l = 0, int r = len) {
	if (l >= y || r <= x) return 0;
...
int main() {
	while (scanf("%d", &k) && k > 0) {
        ...
        */
		...
		for (int i = 0; i < len; i++) if (sa[i].color > -1) update(i, 1 << sa[i].color);
/*		for (int i = k; i < len; i++) {
...
*/		for (int i = 0; i < len; i++) if (sa[i].color > -1 && lcp[i]) {
			int x = lef[i], y = rig[i];
			assert(x >= 0);
			assert(y < len);
			fprintf(stderr, "x: %d y: %d\n", x, y);
			long long mask = query(x, min(y + 2, len));
			st.insert(mask);
		}
		for (int i = 0; i < len; i++) if (start[sa[i].i]) {
			if (lcp[i] >= size[sa[i].i]) continue;
			if (i > 0 && lcp[i - 1] >= size[sa[i].i]) continue;
			assert(sa[i].color > -1);
			st.insert(1 << sa[i].color);
		}
		printf("%lld\n", (long long)st.size());
